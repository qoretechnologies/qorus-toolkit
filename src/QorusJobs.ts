...
class QorusJobs {
   ...
   */
  async get(jobId?: JobId): Promise<IJob[] | IJob> {
    ...
    const error = response as IErrorAxiosParams;
   ...
   */
  async enable(jobId: JobId): Promise<IEnableDisableJobResponse> {
    const requestPath = getRequestPath([this.browsePath, jobId?.toString()]);
    const requestParams = {
      action: 'enable',
    };
   ...
   * @returns EnableJobResponse with jobId, name, version of the job
   */
  async disable(jobId: JobId): Promise<IEnableDisableJobResponse> {
    const requestPath = getRequestPath([this.browsePath, jobId?.toString()]);
    const requestParams = {
      action: 'disable',
    };
   ...
   * @returns EnableJobResponse with jobId, name, version of the job
   */
  async killProcesses(jobId: JobId): Promise<IKillProcessesResponse> {
    const requestPath = getRequestPath([this.browsePath, jobId?.toString()]);
    const requestParams = {
      action: 'kill',
    };
...
type TJobId = number | string;

// Job source code
type TJobSourceCode = string;
