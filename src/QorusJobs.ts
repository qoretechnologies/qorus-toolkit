...
class QorusJobs {
   ...
   */
  async get(jobId?: JobId): Promise<Job[] | Job> {
    const requestPath = getRequestPath([this.browsePath, jobId?.toString() ?? '']);
    const response = await QorusRequest.get({ path: requestPath });

    const responseAsJobs = response as AxiosResponse;
    const error = response as ErrorAxiosParams;

    if (error.status) {
      throw new ErrorAxios(error);
    }
    const responseData = responseAsJobs?.data as Job[];
   ...
   */
  async delete(jobId: JobId): Promise<undefined> {
    const requestPath = getRequestPath([this.browsePath, jobId?.toString() ?? '']);
   ...
   */
  async update(jobId: JobId, jobSourceCode: JobSourceCode, metadata?: Record<string, any>): Promise<UpdateJobResponse> {
    const requestPath = getRequestPath([this.browsePath, jobId?.toString() ?? '']);
    const requestData = {
      code: jobSourceCode,
      metadata: metadata,
    ...
    const responseAsData = response as AxiosResponse;
    const jobUpdateReponse = responseAsData.data as UpdateJobResponse;
   ...
   */
  async enable(jobId: JobId): Promise<EnableDisableJobResponse> {
    const requestPath = getRequestPath([this.browsePath, jobId?.toString()]);
    const requestParams = {
      action: 'enable',
    };
    ...
    const responseAsData = response as AxiosResponse;
    const jobEnableReponse = responseAsData.data as EnableDisableJobResponse;
   ...
   * @returns EnableJobResponse with jobId, name, version of the job
   */
  async disable(jobId: JobId): Promise<EnableDisableJobResponse> {
    const requestPath = getRequestPath([this.browsePath, jobId?.toString()]);
    const requestParams = {
      action: 'disable',
    };
    ...
    const responseAsData = response as AxiosResponse;
    const jobDisableReponse = responseAsData.data as EnableDisableJobResponse;
   ...
   * @returns EnableJobResponse with jobId, name, version of the job
   */
  async killProcesses(jobId: JobId): Promise<KillProcessesResponse> {
    const requestPath = getRequestPath([this.browsePath, jobId?.toString()]);
    const requestParams = {
      action: 'kill',
    };
    ...
    const responseAsData = response as AxiosResponse;
    const jobKillReponse = responseAsData.data as KillProcessesResponse;
...
export interface EnableDisableJobResponse {
...
// Job source code
export type JobSourceCode = string;
